...
We call a hash function $H$ **hiding**, if when a secret value $r$ is choosen from a probability distribution that has *high min-entropy* (i.e the distribution is very spread out, and it is not predictable which output the random variable has), then given $H(r || x)$ it is infeasible to find $x$. ($ a || b $ denotest the concatenation of $a$ and $b$)
